...
The paper FlashHead proposes a technique to approximate the head projection with approximate nearest neighbors (ANN) @flashhead2026. This allows the transformer to find the next token to generate without calculating a score for all tokens in the vocabulary. However, the key framing in this thesis is that even though you make the head _zero_ in computational cost, the total speedup is bounded by the fraction of total compute the head represents. This is the Amdahl's law perspective that motivates this thesis. So if the head is 20% of the total compute to generate the next token, the total speedup by approximating the head cannot exceed $1/0.8 = 1.25$.
...
The overall methodology for the project is to implement the inference setups in PyTorch. Different configurations with different hyper-parameters are measured how well they perform for their targeted task. Metrics like cross-entropy, KL divergence, Top1 match and acceptance rate is used to measure performance. The project is available as an open source repository at #link("https://github.com/HugoOlsson/SkipSearchSpec")[SkipSearchSpec]#footnote[Repository URL: #link("https://github.com/HugoOlsson/SkipSearchSpec")[https://github.com/HugoOlsson/SkipSearchSpec].]. All measurements presented in this thesis uses the same measurement-pipeline so that they always follow a shared structure, includes their git commit/tag and stores the raw data in the repository. By including the exact commit for each result, the reader can always visit the exact state of the project for when a measurement was performed.
...
== Datasets
The datasets used to train the HVC-bridge are 18% `HuggingFaceTB/cosmopedia-100k` @huggingface_cosmopedia100k, 18% `codelion/fineweb-edu-1B` @huggingface_finewebedu1b, 41% `MBZUAI/LaMini-instruction` @huggingface_lamini_instruction, 18% `flytech/python-codes-25k` @huggingface_pythoncodes25k, and 5% `roneneldan/TinyStories` @huggingface_tinystories. All training runs use a single epoch. This is to maximize the amount of examples seen given the allocated compute, but also to get KL/CE/top1 training graphs that don't include progress where the module has seen the data before. The datasets are selected to work with the completion-style training setup and to be a good foundation to handle a spectrum of different prompts.
...
Algorithm @alg:strict-equal-lm-head-clustering is used to produce the clustering.
...
Literal examples from the three prompt sets are shown in Listing @lst-main-prompt-examples. The concrete set covers bounded tasks with short, checkable outputs, while the open-ended and Python-diverse sets include prompts where the continuation is naturally longer.
...
#figure(
  text(size: 8.5pt)[
    #table(
      ...
      table.cell(rowspan: 2)[`concrete-completion-style`],
      [Example 1],
      [
      ...
      table.cell(rowspan: 2)[`open-ended`],
      [Example 1],
      ...
      table.cell(rowspan: 2)[`python-diverse-completion-style`],
      [Example 1],
      [
  ...
  caption: [Example prompts from the three prompt sets used in the main self-speculation benchmark.],
  kind: "listing",
  supplement: [Listing],
...
=== Why not test with fewer skipped layers
...
$
This gives $d_1 = 6.94%$ for the (1,1) + ANNH drafter. With 65% top-1 accuracy, this baseline gives a theoretical speedup of $1.474 times$ for block size 1. For greedy self-speculation with block size 1, top-1 agreement is approximately the same as the acceptance rate because each speculative round drafts only one token. Table @tab-less-skipped-required-top1-block1 therefore shows what acceptance rate a more expensive $(N, N)$ drafter would need to match that baseline speedup.
...
use the setup times in Table @tab-total-training-time to produce the HVC-bridge and the ANNH. 
...
// Source data for the speedup-vs-model-size figure:
