...
def get_dataset_mix_prompt_aligned(
    ...
    mix: list[tuple[DatasetSpec, float, float]] = [
        ...
        (DATASET_SPEC_FINEWEB_EDU_1B, 0.25, 1.3*buffer_adjuster),

        # General instruction / answer formatting.
        (DATASET_SPEC_ALPACA_FORMATTED, 0.10, 6*buffer_adjuster),
        (DATASET_SPEC_DOLLY_15K_FORMATTED, 0.10, 6.0*buffer_adjuster),

        # Science / factual QA.
        (DATASET_SPEC_SCIQ_FORMATTED, 0.09, 6.0*buffer_adjuster),
        (DATASET_SPEC_SQUAD_FORMATTED, 0.08, 6.0*buffer_adjuster),

        # Math solution formatting.
        (DATASET_SPEC_GSM8K_FORMATTED, 0.09, 6.0*buffer_adjuster),
        (DATASET_SPEC_METAMATHQA_40K_FORMATTED, 0.08, 6.0*buffer_adjuster),

        # Code completion / small Python tasks.
        (DATASET_SPEC_PYTHON_CODES_25K, 0.06, 6.8*buffer_adjuster),
...
def get_dataset_mix_v2(
    ...
    mix: list[tuple[DatasetSpec, float, float]] = [
        ...
        (DATASET_SPEC_COSMOPEDIA_100K, 1, 2.5),
